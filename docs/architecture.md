...
- `.trivyignore` holds repo-wide check IDs — CVE, secret and misconfiguration
  IDs are all accepted — that are wrong or unfixable for *every* repository.
  Each entry carries a comment stating why (for example `AWS-0079`, because
  Trivy cannot resolve an `Fn::If`-guarded `StorageEncrypted`). An ID here
  suppresses its check repo-wide, so repo-specific exclusions do not belong in
  it
- `trivy.yaml` holds `scan.skip-dirs`. The block between the
  `ghb:excluded-dirs:start` / `ghb:excluded-dirs:end` sentinels is generated by
  github-build from `config/languages.yaml` and must not be hand-edited;
  project-specific directories go outside the sentinels so they survive a
  rebuild. Vulnerability IDs never go here
...
- `run_shellcheck.sh`: finds every `*.sh` file plus any other file with a shell
  ...
  by `linters/shellcheck`; unit-tested by `linters/tests/run_shellcheck.bats`
...
- `build.yml`: generated by github-build with `--skip_slack`, so it carries no
  Slack-notification job. It runs the detected linters, the `licenses` job (the
  `soup` action with `--no_prompt --gha`, see
  [Software of Unknown Provenance](#software-of-unknown-provenance)), the Slack
  Jest suite (`js_unit_tests`) and the bats
...
- `external-actions-bump.yml`: hand-maintained (explicitly not generated)
  ...
  PR for human review
- `auto-approve.yml`: generated by github-build (`auto_approve_manager.rb`), so
...
See [soup.md](soup.md) for the complete list of third-party dependencies: the Slack
action's npm packages and every GitHub Action referenced by `uses:` in the
composite actions and workflows.
...
are read from `slack/package-lock.json` for npm entries and from the `uses:` refs
and the GitHub API for GitHub Action entries, so they are never duplicated here.
...
- **Risk Level** (per IEC 62304): Low — cannot lead to harm; Medium — reversible
  ...
  and touch no deployment credential or artifact. A GitHub Action is Medium when
  it receives a deployment credential (SSH key, AWS credentials, registry login)
  or builds, caches or attests a shipped artifact, and Low otherwise (linters,
  toolchain installers, test services)
...
- **Verification Reasoning** — why this package over the alternatives (for
  example "Official GitHub Actions SDK"), not a popularity argument. A
  community-maintained action has no official-maintainer argument, so its entry
  records the control that applies instead: its ref is bumped and reviewed
  through the weekly `external-actions-bump` PR

**Scope:** `.soup.json` holds two entry classes:

- **npm (`JS`)** — the runtime dependency closure of the only manifest in the
  repository, `slack/package.json`: the two direct dependencies
  (`@actions/core`, `axios`) plus their transitives. Build-only
  `devDependencies` (`jest`, `@vercel/ncc`) and their transitives are
  deliberately excluded: they are never bundled into `dist/index.js` and never
  execute on a consumer's runner
- **GitHub Actions (`GHA`)** — every `owner/repo` referenced by `uses:` in an
  `action.yml` or `.github/workflows` file, found by soup's `--gha` scan, with
  all of its refs as the version. The external actions run on a consumer's
  runner in the same job as `SSH_KEY`, `GH_PAT` and AWS credentials, so they
  carry the same review fields as a library. `cloud-officer/ci-actions` is
  listed for the references the generated workflows make to this repository;
  local `./` references are not recorded

A new `uses:` repository without an entry fails the `licenses` job, because it
runs with `--no_prompt`. A bump that only changes a ref keeps the entry's review
fields; run `soup --gha` locally in the bump PR to refresh the recorded versions.

The list is validated on four axes: accuracy (Requirements match the real call
sites), completeness (every runtime package resolved in
`slack/package-lock.json` and every referenced action repository is present),
staleness (packages dropped from the runtime closure and actions no longer
referenced are removed), and Risk Level appropriateness.
...
The build workflow grants minimal permissions at the workflow level:
...
from the token passed to the step, not from the workflow permission block.
