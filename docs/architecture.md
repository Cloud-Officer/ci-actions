...
**Scope:** `.soup.json` covers the runtime dependency closure of the only
manifest in the repository, `slack/package.json` — the two direct dependencies
(`@actions/core`, `axios`) plus their transitives, 33 entries in total.
Build-only `devDependencies` (`jest`, `@vercel/ncc`) and their transitives are
...
`slack/package-lock.json` is present), staleness (packages dropped from the
runtime closure are removed), and Risk Level appropriateness.
